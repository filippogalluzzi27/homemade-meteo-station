...
Schermatura Stevenson (legno); altezza 2 m dal suolo.
...
| **Pluviometro a bascula** | Pioggia (interrupt su impulso) | `GPIO2`, `INPUT_PULLUP` |

> **Costante calibrazione pluviometro**: `RAIN_PER_TIP = 0.3 mm/tip`.
...
`WiFi`, `HTTPClient`, `Wire`, `SPI`, `SD`, `RTClib`, `Adafruit_SHT31`, `esp_task_wdt`
...
- **Pioggia**: conteggio impulsi via ISR con **debounce 400 ms**; calcolo mm/blocco e **intensità max (mm/h)** tra impulsi consecutivi (entro 1h), con filtro anti-spike (`MAX_VALID_INTENSITY = 500`).
...
Script **`analisi_meteo.R`** — solo pacchetti **base R**, nessuna dipendenza esterna.

### Requisiti dati

Esporta il feed da ThingSpeak in `data/raw/feeds.csv` con colonne `created_at`, `entry_id`, `field1…field5`.

### Esecuzione

```bash
Rscript analisi_meteo.R
```
...
.
├── scripts/
│   ├── stazione_meteo.ino      # sketch ESP32 per Arduino
│   └── analisi_meteo.R         # analisi + grafici
├── data/
│   └── raw/
